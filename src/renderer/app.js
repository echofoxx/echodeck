(() => {
  ...
  const STORAGE_KEY = 'echodeck:v0.1.0';
  ...
  function streamingTrackFromUrl(rawUrl) {
    const url = rawUrl.trim();
    if (!url) return null;
    const lower = url.toLowerCase();
    if (lower.includes('youtube.com') || lower.includes('youtu.be')) {
      const videoId = parseYouTubeId(url);
      if (!videoId) throw new Error('Could not read a YouTube video ID from that URL.');
      return {
        id: `youtube-${videoId}`,
        sourceType: 'youtube',
        sourceUrl: url,
    ...
    if (lower.includes('soundcloud.com')) {
      return {
        id: `soundcloud-${btoa(url).replace(/=+$/, '')}`,
    ...
    throw new Error('This MVP supports YouTube and SoundCloud URLs. Spotify and Apple Music are planned SDK integrations.');
  }

  function addStreamingUrl(url) {
    try {
      const track = streamingTrackFromUrl(url);
      dedupeAddTracks([track]);
      setState(draft => {
        if (!draft.queue.includes(track.id)) draft.queue.push(track.id);
        if (draft.queueIndex < 0) draft.queueIndex = draft.queue.length - 1;
      });
    } catch (error) {
      toast(error.message);
    }
  ...
  function renderPlaybackState() {
    ...
    els.playBtn.textContent = localPlaying || (isPlaying && currentTrack()?.sourceType !== 'local') ? '⏸' : '▶';
    els.recordDisc.classList.toggle('playing', localPlaying || (isPlaying && currentTrack()?.sourceType !== 'local'));
  ...
  function render() {
    ...
    els.runtimeLabel.textContent = isElectron ? 'macOS Local App Ready' : 'Browser Preview Mode';
  ...
  function wireEvents() {
    ...
    els.confirmStreamAdd.addEventListener('click', (event) => {
      ...
      addStreamingUrl(els.genericStreamUrl.value);
    ...
    els.spotifyPlanBtn.addEventListener('click', () => showInfo('Spotify Integration Plan', 'Phase 2 will add Spotify OAuth, catalog search, playlist import, and Web Playback SDK playback where Spotify allows it. Some users may need Spotify Premium. EchoDeck will not intercept, modify, cache, or re-stream Spotify audio.'));
    els.applePlanBtn.addEventListener('click', () => showInfo('Apple Music Integration Plan', 'Phase 3 will add MusicKit authorization, Apple Music catalog search, user library access, and MusicKit playback. EchoDeck will use official Apple Music APIs and authorized playback only.'));

# EchoDeck v0.2.3 — Adaptive Panel Fit
...
## v0.2.3 adaptive panel fit update

EchoDeck v0.2.3 focuses on preventing overflow in the Now Playing screen. It adds adaptive panel sizing, responsive typography, dynamic title fitting, and stronger clipping rules so metadata no longer bleeds into the Queue panel when the desktop window is narrow or short.
...
The iOS app is not packaged in this release. v0.2.3 prepares the shared UI/theme layer so a future Capacitor shell can reuse the EchoDeck player experience.
